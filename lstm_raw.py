...
#     slice = np.random.randint(1200,3600)
...
#
#
# df_scaled_train = shuffled_df_with_min_length_seq[:int(np.floor(0.8*len(df_scaled)))]
...
# print(df_scaled_test.shape)
#
# df_scaled_test = pd.read_csv('accel_gyro_raw_scaled_shuffled_test.csv')
# df_scaled_train = pd.read_csv('accel_gyro_raw_scaled_shuffled_test.csv')
# #train set (shuffled)
#
# #shuffling
# shuffled_inds = 0
...
#
#
# df_scaled_train = pd.DataFrame(shuffled_df_with_min_length_seq_train, columns=df.columns)
...
# print(X_test.shape)
# pickle.dump([X_train,X_test,y_train,y_test], open("training_test_raw.p","wb"))
X_train, X_test, y_train, y_test =   pickle.load( open("training_test_raw.p", "rb" ) )
if ONLY_ACCEL:
    X_train, X_test = X_train[:,:,:3], X_test[:,:,:3]
if ONLY_GYRO:
    X_train, X_test = X_train[:,:,3:], X_test[:,:,3:]
X_train = X_train[:int(len(X_train)*FRAC_TRAIN),:]
y_train = y_train[:int(len(y_train)*FRAC_TRAIN),:]
