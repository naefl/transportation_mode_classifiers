...
data = pd.read_csv('combined_trainingsdata_butter_5hz_lowpass.csv')
...
if __name__ == '__main__':
    ...
    for i,clf in enumerate(clfs[::2]):
        clf_bus = clfs[1::2][i]
        X_KDA = LinearDiscriminantAnalysis(n_components=5).fit_transform(X, Y)
        y_hats.append(cross_val_predict(clf, X_KDA,Y,cv=10, n_jobs=-1))
        ...
        print(conf_mats[i])
        print(accs[i])
        y_bus_hats.append(cross_val_predict(clf_bus, X_KDA, Y_bus, cv=10, n_jobs=-1))
    ...
    for i,clf in enumerate(clfs[::2]):
        clf_bus = clfs[1::2][i]
        plt.plot(range(46), accs[i*45:(i+1)*45])
        plt.title("".format(retrieve_name(clf)[0]))
